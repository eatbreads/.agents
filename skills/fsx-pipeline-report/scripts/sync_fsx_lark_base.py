...
def ensure_fields(base_token: str, table_id: str, fields: list[dict]) -> None:
    existing = run_json(
        [
            "lark-cli",
            "base",
...
def delete_default_table(base_token: str) -> None:
    tables = run_json(
        ["lark-cli", "base", "+table-list", "--as", "user", "--base-token", base_token]
    )["data"]["items"]
    for table in tables:
...
def clear_table(base_token: str, table_name: str) -> None:
    while True:
        listed = run_json(
            [
                "lark-cli",
...
def upsert_rows(base_token: str, table_name: str, rows: list[dict], field_names: list[str]) -> None:
    for row in rows:
...
def main() -> None:
    ...
    summary_table_id = ensure_table(args.base_token, "汇总信息")
    detail_table_id = ensure_table(args.base_token, "失败步骤明细")
    ensure_fields(args.base_token, summary_table_id, SUMMARY_FIELDS)
    ensure_fields(args.base_token, detail_table_id, DETAIL_FIELDS)
    delete_default_table(args.base_token)
    clear_table(args.base_token, "汇总信息")
    clear_table(args.base_token, "失败步骤明细")
    upsert_rows(
        args.base_token,
        "汇总信息",
        summary_rows,
        ["流水线标题", "流水线 ID", "日期", "最近运行 ID", "运行状态", "失败步骤数", "阻塞步骤数", "首个失败步骤", "流水线链接", "是否需要关注"],
    )
    upsert_rows(
        args.base_token,
        "失败步骤明细",
        detail_rows,
        ["日期", "流水线标题", "流水线 ID", "运行 ID", "运行状态", "失败步骤", "失败状态", "任务链接"],
    )
    print(
        json.dumps(
            {
